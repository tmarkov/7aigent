...
class TimerEnvironment:
    ...
    def handle_command(self, cmd: CommandText) -> CommandResponse:
        ...
        if command == "start":
            if self._running:
                return CommandResponse("Timer is already running", processed=False)
            ...
            if not self._running:
                return CommandResponse("Timer is not running", processed=False)
            ...
            return CommandResponse(
                f"Timer stopped. Elapsed: {elapsed:.2f}s", processed=True
            )
            ...
            self._elapsed = 0.0
            self._running = False
            return CommandResponse("Timer reset", processed=True)
            ...
            if self._running:
                ...
                return CommandResponse(
                    f"Timer stopped. Total: {self._elapsed:.2f}s", processed=True
                )
