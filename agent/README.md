...
3. Executes tool calls (`julia_repl`, `git_stage`, `git_commit`), feeds results back to the LLM
...
calls, refreshes the persistent CodeTree workspace view before Julia/Git tool
execution so Git overlays stay current, and restores `julia_defs.jl` plus
`julia_state.jls` on `resume`.
